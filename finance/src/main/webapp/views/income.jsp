...
<body background="${APP_PATH }/static/image/bg.jpg">
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        ...
	        <h4 class="modal-title" id="myModalLabel">更新收入信息</h4>
	      </div>
	      ...
	      <div class="modal-footer">
	        ...
	        <button type="button" class="btn btn-primary" id="update_income_btn">更新</button>
	      </div>
	    </div>
	  </div>
	</div>
	...
	<div class="modal fade" id="addIncomeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	        <h4 class="modal-title" id="myModalLabel">新增收入信息</h4>
	      </div>
	      <div class="modal-body">
	      	<form class="form-horizontal" id="addForm">
			  <div class="form-group">
	      ...
	      <div class="modal-footer">
	        <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
	        <button type="button" class="btn btn-primary" id="save_income_btn">保存</button>
	      </div>
	    </div>
	  </div>
	</div>
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-3 col-md-offset-9">
				<button class="btn btn-primary"  id="add_income_btn"  style="margin-left:15px">新增</button>
		...
		<div class="row">
			<div class="col-md-11 col-md-offset-1" style="font-size:16px;">
				<table class="table table-hover" id="incomeShow">
					<thead>
						<tr>
	...
	<script type="text/javascript">
		...
		function to_page(pn){
			...
			$.ajax({
				url:"${APP_PATH }/income/show",
				data:{"username":username,"pn":pn},
				type:"GET",
				success:function(result){
					//1、解析并显示数据
					build_income_table(result);
					//2、解析并显示分页信息
		...
		function build_income_table(result){
			//清空table表格
			$("#incomeShow tbody").empty();
			
			var incomes = result.map.pageInfo.list;
			$.each(incomes,function(index,item){
				...
				editBtn.attr("edit-id",item.incomeid);
				var delBtn =  $("<button></button>").addClass("btn btn-danger btn-sm delete_btn")
								.append($("<span></span>").addClass("glyphicon glyphicon-trash")).append("删除");
				//为删除按钮添加一个自定义的属性来表示当前删除的员工id
				delBtn.attr("del-id",item.incomeid);
		...
		$("#add_income_btn").click(function(){
			...
			reset_form("#addIncomeModal form");
		});
		
		
		
		$("#save_income_btn").click(function(){	
			$.ajax({
				url:"${APP_PATH}/income/save",
		...
		$(document).on("click",".edit_btn",function(){
			$("#update_income_btn").attr("edit-id",$(this).attr("edit-id"));
			getIncome($(this).attr("edit-id"));	
		...
		function getIncome(incomeid){
			$.ajax({
				...
				success:function(result){
					var income = result.map.income;
					var date = dateFormat(income.date);
					$("#usernamestatic").text(income.username);
					$("#inputUpdateDate").val(date);
					$("#inputUpdateMoney").val(income.money);
					$("#inputUpdateNote").val(income.note);
				}
			});
		}
		
		$("#update_income_btn").click(function(){
			$.ajax({
				url:"${APP_PATH}/income/update",
				data:{
					"incomeid":$(this).attr("edit-id"),
					"username":username,
		...
		$(document).on("click",".delete_btn",function(){
			...
			if(confirm("确认删除吗？")){
				//确认，发送ajax请求删除即可
				del_income($(this).attr("del-id"));
			}
		});
		
		function del_income(incomeid){
			$.ajax({
				url:"${APP_PATH}/income/delete",
				type:"POST",
				data:"incomeid="+incomeid,
				success:function(){
					alert("删除成功");
